...
public class MenuDriven {
	...
	void insert_into_employee() {
		...
		for (int i = 0; i < count; i++) {
			...
			dept = sc.nextLine();
			try {
				java.sql.PreparedStatement PS = Sqlutil.getConn().prepareStatement(SQL_INSERT);
				PS.setInt(1, eno);
				PS.setString(2, name);
				PS.setInt(3, salary);
				PS.setString(4, desg);
	...
	static void ViewAllEmp() {
		...
		try {
			java.sql.PreparedStatement PS1 = Sqlutil.getConn().prepareStatement(SQL_READ);
			// PS1.setInt(1, enumber);
	...
	static void Remove() {
		...
		try {
			java.sql.PreparedStatement PS2 = Sqlutil.getConn().prepareStatement(SQL_DELETE);
	...
	static void clearData() {
		try {
			...
			java.sql.Statement st = Sqlutil.getConn().createStatement();
	...
	public void changeSalary() {
		...
		try {
			java.sql.PreparedStatement PS3 = Sqlutil.getConn().prepareStatement(SQL_UPDATE);
			PS3.setInt(1, newsal);
			PS3.setInt(2, eno);
	...
	static void searchEmployee() {
		...
		try {
			java.sql.PreparedStatement PS4 = Sqlutil.getConn().prepareStatement(SQL_READ + "WHERE ENO=SEARCH");
	...
	public void deptWise() {
		try {
			java.sql.PreparedStatement PS5 = Sqlutil.getConn().prepareStatement("SELECT * FROM EMPLOYEE ORDER BY DEPT");
	...
	static void SortEmployee() {
		...
		try {
			if (x == 1) {
				java.sql.PreparedStatement ps = Sqlutil.getConn().prepareStatement( "SELECT * FROM EMPLOYEE ORDER BY ENO");
				rs = ps.executeQuery();
			} else if (x == 2) {
				java.sql.PreparedStatement ps = Sqlutil.getConn().prepareStatement( "SELECT * FROM EMPLOYEE ORDER BY NAME");
				rs = ps.executeQuery();
			} else if (x == 3) {
				java.sql.PreparedStatement ps= Sqlutil.getConn().prepareStatement("SELECT * FROM EMPLOYEE ORDER BY DEPT");
				rs = ps.executeQuery();
			} else if (x == 4) {
				java.sql.PreparedStatement ps = Sqlutil.getConn().prepareStatement( "SELECT * FROM EMPLOYEE ORDER BY SALARY");
				rs = ps.executeQuery();
			} else if (x > 4) {
